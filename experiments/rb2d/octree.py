...
class Octree():
    ...
    def __init__(self, data_dir="./data", data_filename="rb2d_ra1e6_s42.npz", data=None, mean=None, std=None, crop_size=(16,128,128), n_min_vol_per_crop=1,div_method="std"):
        """
        Initialize Octree
        Args:
        ...
        if self.div_method == "mean":
            self.std = mean

        octree_name = os.path.join(self.data_dir, self.data_filename+'.octree.pkl')
        octree_leaves_name = os.path.join(self.data_dir, self.data_filename+'.leaves.pkl')
    ...
    def build_octree(self):
        ...
        if not self.data.any():
            ...
            self.mean = np.mean(self.data, axis=(1, 2, 3))
            self.std = np.std(self.data, axis=(1, 2, 3))   
        size_t = self.data.shape[1]
        ...
        self.depth = int(np.log2(max(size_t / self.crop_size[0], size_z / self.crop_size[1], size_x / self.crop_size[2]))+0.5) + self.n_min_vol_per_crop - 1
        ...
        f = open(os.path.join(self.data_dir, self.data_filename+'.octree.pkl'), 'wb')
        f.write(pickle.dumps(self.root)) # save octree
        f = open(os.path.join(self.data_dir, self.data_filename+'.leaves.pkl'), 'wb')
    ...
    def create_node(self, node, new_node_vertex1, new_node_vertex2):
        ...
        new_std = np.std(new_data, axis=(1, 2, 3))
        ...
        if self.depth_now < self.depth and (new_std[0] > self.std[0] or new_std[1] > self.std[1] or new_std[2] > self.std[2] or new_std[3] > self.std[3]):
...
if __name__ == '__main__':
    ...
    my_octree = Octree("./data", "test.npz", data, mean, std, crop_size, n_min_vol_per_crop) 
